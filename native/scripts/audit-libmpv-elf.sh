...
usage() {
  echo '用法：audit-libmpv-elf.sh --input <libmpv.so> --output <文件> [--allow <库名>]... [--require <库名>]... [--forbid <库名>]... [--max-bytes <字节>] [--runtime-dir <配套 FFmpeg 库目录>]' >&2
  exit 2
}
...
runtime_symbols_file="$temp_dir/runtime-symbols.txt"
: > "$runtime_symbols_file"
if [ -n "$runtime_dir" ]; then
  for runtime in "$runtime_dir"/libavcodec.so.62 "$runtime_dir"/libavutil.so.60; do
    [ -f "$runtime" ] || { echo "缺少配套 runtime：$runtime" >&2; exit 1; }
    "$nm_tool" -D --defined-only "$runtime" | awk '{print $NF}' >> "$runtime_symbols_file"
  done
fi
file_bytes="$(wc -c < "$input" | tr -d ' ')"
python3 - "$output" "$needed_file" "$symbols_file" "$undefined_symbols_file" "$runtime_symbols_file" "$machine" "$file_bytes" "$max_bytes" "${#allows[@]}" "${#requires[@]}" ${allows[@]+"${allows[@]}"} ${requires[@]+"${requires[@]}"} ${forbids[@]+"${forbids[@]}"} <<'PY'
import json
import pathlib
import re
import sys
output, needed_path, symbols_path, undefined_symbols_path, runtime_symbols_path, machine, file_bytes, max_bytes, allow_count, require_count, *libraries = sys.argv[1:]
...
runtime_symbols = {symbol.replace('@@', '@') for symbol in pathlib.Path(runtime_symbols_path).read_text().splitlines()}
unresolved_ohcodec_symbols = sorted(set(private_ohcodec_symbols) - runtime_symbols)
size_exceeded = int(max_bytes) > 0 and int(file_bytes) > int(max_bytes)
architecture_valid = machine in ('AArch64', 'ARM aarch64')
failed = unauthorized or missing_required or forbidden_needed or embedded_symbols or unresolved_ohcodec_symbols or size_exceeded or not architecture_valid
report = {
    'schemaVersion': 2,
    ...
    'unresolvedPrivateOhcodecSymbols': unresolved_ohcodec_symbols,
...
if failed:
    ...
    if unresolved_ohcodec_symbols:
        detail.append('存在未解析的私有 OHCodec 符号：' + ', '.join(unresolved_ohcodec_symbols[:20]))
    if size_exceeded:
