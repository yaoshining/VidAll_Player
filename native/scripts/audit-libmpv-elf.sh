...
usage() {
  echo '用法：audit-libmpv-elf.sh --input <libmpv.so> --output <文件> [--allow <库名>]... [--forbid <库名>]...' >&2
  exit 2
}
...
python3 - "$output" "$needed_file" "$symbols_file" "${#allows[@]}" "${allows[@]-}" "${forbids[@]-}" <<'PY'
import json
import pathlib
import sys
output, needed_path, symbols_path, allow_count, *libraries = sys.argv[1:]
allow_count = int(allow_count)
allowed = libraries[:allow_count]
forbidden = libraries[allow_count:]
...
report = {
    'schemaVersion': 1,
    'status': 'passed' if not unauthorized and not forbidden_needed else 'failed',
...
if unauthorized or forbidden_needed:
    detail = []
    if unauthorized:
