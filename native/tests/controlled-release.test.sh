...
assert 'brew install wget rustup-init' in workflow, '真实 libmpv 构建必须通过受控 runner 安装 wget 与 rustup-init'
assert 'rustup-init -y --profile minimal --default-toolchain stable' in workflow, '真实 libmpv 构建必须初始化 Rust 工具链'
