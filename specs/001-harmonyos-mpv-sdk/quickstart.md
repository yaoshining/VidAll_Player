...
本指南先验证可安装但不可发布的验证构件，再验证由其派生的候选制品；不代表当前仓库已经发布或通过真机认证。本 Issue 的 SMB 验证仅覆盖业务层 localhost HTTP proxy 与匿名 lease；当前不测试或声明直接 `smb://`。直接 SMB 需在后续独立 Issue 完成 `libsmbclient` 受控构建、许可证/ELF 审计和 ARM64 TV 证据后另行定义验证步骤。
