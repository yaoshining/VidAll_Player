...
| `ProxyLease` | 匿名 `proxyLeaseId`、状态、续期时间、释放请求/确认、脱敏原因 | SDK 只消费业务层已启动的 loopback HTTP 代理；状态为 `acquired`、`renewed`、`releaseRequested`、`released`、`expired` 或 `cleanupFailed`。SDK 不实现 SMB 或启动代理。 |
